...
impl QuinnEndpoint {
    ...
    fn process_connection_event(
        ...
        let Some((connection, _)) = self.connection_mut(QuinnConnectionId(handle), context) else {
            error!(
...
impl Endpoint for QuinnEndpoint {
    ...
    fn connection(&self, id: ConnectionId<Self>) -> Option<&Self::Connection> {
    ...
    fn connection_mut<'c>(
        ...
        context: &'c mut Self::Context,
    ) -> Option<(&mut Self::Connection, &'c mut QuinnContext)> {
        self.connections
            .get_mut(&id)
